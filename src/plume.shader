...
float shape(vec2 coord, float radius) {
	...
	return clamp(sqrt(1.0 - dist * dist), 0.0, 1.0);
}
void fragment() {
	vec2 coord = UV * 8.0;
	vec2 fbmcoord = coord / 6.0;

	float plume_shape = shape(UV, 0.35);
	plume_shape += shape(UV, 0.15);
	float noise1 = noise(coord + vec2(TIME * 0.85, TIME * 12.0));
	float noise2 = noise(coord + vec2(TIME * 1.0, TIME * 10.0));
	float combined_noise = (noise1 + noise2) / 2.0;

	float fbm_noise = fbm(fbmcoord + vec2(0.0, TIME * 3.0));
	fbm_noise = overlay(fbm_noise, UV.y);

	float everything_combined = combined_noise * fbm_noise * plume_shape;
